...
def deduplicate_existing_heatmap_records(franchise_id):
    """Collapse existing repeated imports while preserving the newest supplied details."""
    existing_records = (
        HeatmapRecord.query
        .filter_by(franchise_id=franchise_id)
        .order_by(HeatmapRecord.id.asc())
        .all()
    )
    keepers = {}
    removed = 0
    for record in existing_records:
        identity = heatmap_record_identity(record)
        keeper = keepers.get(identity)
        if keeper is None:
            keepers[identity] = record
            continue
        merge_heatmap_record(keeper, record)
...
def import_excel():
    ...
    try:
        ...
        if not records:
            ...
            return redirect(url_for("heatmap.index"))
        removed_duplicates = deduplicate_existing_heatmap_records(franchise_id)
        ...
        detail = (
            ...
            f"Removed existing duplicates: {removed_duplicates}. "
